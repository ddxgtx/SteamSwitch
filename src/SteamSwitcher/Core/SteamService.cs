    ...
    public class SteamService
    {
        ...
        public async Task<bool> CloseSteamAsync(bool silent = false)
        {
            ...
            try
            {
                ...
                if (IsSteamRunning())
                {
                    AppLogger.Info("Steam still running after -shutdown, force killing.");
                    if (silent) HideSteamWindows();
                ...
                for (var i = 0; i < 5; i++)
                {
                    ...
                    AppLogger.Info($"Steam still running, retry kill attempt {i + 1}.");
                    if (silent) HideSteamWindows();

                    var retrySteam = Process.GetProcessesByName("steam");
                    var retryWeb = Process.GetProcessesByName("steamwebhelper");
                    var retryProcs = retrySteam.Concat(retryWeb).ToList();
                    foreach (var process in retryProcs)
                    {
                        try
                        {
                            process.Kill();
